...
def load_get_list_manifests(
    ...
    has_stored = (
        "Action5Pro_Nomal" in headers
        and any(
            str(sheet.cell(row_idx, headers["Action5Pro_Nomal"]).value or "").strip()
    ...
    for row_index in range(3, sheet.max_row + 1):
        ...
        if has_stored:
            # 全量从 xlsm 读取，不扫目录
            normal_name = str(sheet.cell(row_index, headers["Action5Pro_Nomal"]).value or "").strip()
            night_name = str(sheet.cell(row_index, headers["Action5Pro_Night"]).value or "").strip()
            normal = Path(dji_nomal_dir) / normal_name if normal_name else Path(".")
            night = Path(dji_night_dir) / night_name if night_name else Path(".")
        else:
            # 新批次，从目录按位置匹配
