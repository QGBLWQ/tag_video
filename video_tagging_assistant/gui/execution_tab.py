...
class ExecutionTab(QWidget):
    ...
    def _refresh_port_status(self) -> None:
        ...
        try:
            ...
            r = subprocess.run(
                ["adb", "shell",
                 "for p in /proc/[0-9]*; do tr '\\0' ' ' < $p/cmdline 2>/dev/null | grep -q 'nc -l' && echo $(basename $p); done | head -5"],
                capture_output=True, text=True, timeout=5, **kw)
    ...
    def _clear_stale_ports(self) -> None:
        ...
        try:
            subprocess.run(
                ["adb", "shell",
                 "for p in /proc/[0-9]*; do tr '\\0' ' ' < $p/cmdline 2>/dev/null | grep -q 'nc -l' && kill $(basename $p) 2>/dev/null; done; "
