...
- `temp_path` 若存在有效 RK 目录，则优先作为对齐候选来源；否则回退到 `dut_root`。
- RK 候选目录名支持纯数字和数字后缀 `x`；缺少 `jpg/jpeg` 预览图的目录会被排除，并记录到对齐页日志。
- `获取列表.RK_raw` 为空的行会进入 `对齐` 页待处理队列；已填写的行默认不进入，需通过对齐页的重写入口重新加载。
- `打标` 与 `对齐` 可以并行进行，但只有“整批打标完成”且“整批对齐完成”后，`审核` 页才会解锁。
- DJI 预览缓存保存在 `artifacts/alignment_previews/<case_id>_<digest>/`，其中 `<digest>` 由 normal/night 视频路径计算得到，按 `normal/` 与 `night/` 子目录组织。
...
- [ ] `temp_path` — 若使用本地暂存 RK 原始目录，确认根路径正确，且包含按 RK 后缀分组的非空子目录
...
## Alignment Preview Addendum

| Field | Type | Example | Description |
|------|------|--------|------|
| `ffprobe_exe` | string | `"ffprobe.exe"` | Command used to probe DJI videos for alignment preview generation. This can be a PATH name or a full executable path. |
| `ffmpeg_exe` | string | `"ffmpeg.exe"` | Command used to extract DJI preview frames for alignment. This can be a PATH name or a full executable path. |
| `alignment_preview_frame_count` | int | `30` | Maximum number of preview frames generated per DJI video. |
| `alignment_preview_skip_frames` | int | `2` | Fixed number of source frames skipped after each selected frame. `2` means selecting frames `0, 3, 6, 9...`. |
| `alignment_preview_workers` | int | `2` | Maximum number of background preview preparation tasks running in parallel. |

- The alignment tab starts background DJI preview preparation when a batch is loaded; it does not wait for per-case clicks before running ffprobe/ffmpeg.
- Reloading a batch or closing the main window waits for the active preview worker to stop cleanly so overlapping workers do not continue running.
- When `temp_path` has no usable RK directory and `dut_root` is not a real local directory, the GUI scans remote RK candidates through `adb shell find` and pulls preview images on demand. Remote alignment therefore requires `adb_exe` to be available.
